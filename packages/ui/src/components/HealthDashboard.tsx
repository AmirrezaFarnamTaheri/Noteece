          ...
          <Stack gap={4}>
            <Group justify="space-between">
              ...
              <Text
                size="xs"
                fw={600}
                c={progress >= 100 ? "green" : undefined}
              >
                {Math.round(progress)}%
              </Text>
            </Group>
...
export function HealthSummaryRing({
  metrics,
  goals,
}: Pick<HealthDashboardProps, "metrics" | "goals">) {
  const metricTypes: HealthMetric["type"][] = [
    "mood",
    "sleep",
    "water",
  ...
  const overallProgress =
    metricTypes.reduce((sum, type) => sum + getGoalProgress(type), 0) /
    metricTypes.length;
