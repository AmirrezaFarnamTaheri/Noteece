...
mod tests {
    ...
    fn setup_db() -> Connection {
        ...
        conn.execute("CREATE TABLE tasks (id TEXT PRIMARY KEY, title TEXT, status TEXT, updated_at INTEGER, space_id TEXT)", []).unwrap();
        conn
    }
    ...
    fn test_delta_application() {
        ...
        // Verify note exists
        let count: i64 = conn.query_row("SELECT count(*) FROM notes WHERE id = ?", [&note_id], |row| row.get(0)).unwrap();
        assert_eq!(count, 1);
    }
}
