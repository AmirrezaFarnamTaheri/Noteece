...
export default function MusicLabScreen() {
  ...
  const playTrack = async (track: Track) => {
    try {
      ...
      const { sound: newSound } = await Audio.Sound.createAsync({ uri: track.url }, { shouldPlay: true });
  ...
  const playPreviousTrack = () => {
    ...
    const previousIndex = currentIndex === 0 ? filteredTracks.length - 1 : currentIndex - 1;
        ...
        <ScrollView style={styles.scrollView}>
              ...
              >
                <View style={styles.nowPlayingContent}>
                  <View style={styles.nowPlayingInfo}>
                    <Ionicons name={isPlaying ? 'radio' : 'pause-circle'} size={48} color="#FFFFFF" />
                  ...
                  <View style={styles.controls}>
                    ...
                    <TouchableOpacity style={styles.controlButtonPrimary} onPress={togglePlayPause}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterContainer}>
            ...
            {[
              ...
              <TouchableOpacity
                key={genre.key}
                style={[styles.filterButton, filter === genre.key && styles.filterButtonActive]}
                onPress={() => setFilter(genre.key as GenreFilter)}
              >
                <Text style={[styles.filterButtonText, filter === genre.key && styles.filterButtonTextActive]}>
          ...
          <View style={styles.trackList}>
            {filteredTracks.map((track) => (
              <TouchableOpacity
                key={track.id}
                style={[styles.trackCard, currentTrack?.id === track.id && styles.trackCardActive]}
                onPress={() => playTrack(track)}
              >
                <View style={styles.trackInfo}>
                  <View style={[styles.trackIcon, currentTrack?.id === track.id && styles.trackIconActive]}>
                    <Ionicons
                      name={currentTrack?.id === track.id && isPlaying ? 'radio' : 'musical-note'}
                      size={24}
                      color={currentTrack?.id === track.id ? colors.primary : colors.textSecondary}
                    />
                  </View>

                  <View style={styles.trackText}>
                    <Text style={[styles.trackTitle, currentTrack?.id === track.id && styles.trackTitleActive]}>
                      {track.title}
                    </Text>
          ...
          <View style={styles.infoCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.infoText}>
                {MUSIC_LIBRARY.length} tracks across 9 genres • Royalty-free music from Incompetech, Bensound & Free
                Music Archive
              </Text>
              <Text style={[styles.infoText, { marginTop: spacing.xs, fontSize: typography.fontSize.xs }]}>
                🎵 Background playback • 🔄 Auto-play next • 📱 Controls lock screen
              </Text>
            </View>
          </View>
        </ScrollView>
