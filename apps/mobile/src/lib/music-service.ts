...
export function getRandomTrack(genre?: Track['genre']): Track {
  ...
  if (!tracks || tracks.length === 0) {
    throw new Error(`No tracks available${genre ? ' for genre: ' + genre : ''}`);
  }
 ...
 */
export function getRecommendedTracks(currentTrack: Track, limit: number = 3): Track[] {
  const sameGenre = MUSIC_LIBRARY.filter((track) => track.genre === currentTrack.genre && track.id !== currentTrack.id);
 ...
 */
export function createPlaylistForActivity(activity: 'focus' | 'relax' | 'sleep' | 'energize' | 'meditate'): Track[] {
  switch (activity) {
    case 'focus':
      return [...getTracksByGenre('instrumental'), ...getTracksByGenre('classical')];
    case 'relax':
      return [...getTracksByGenre('ambient'), ...getTracksByGenre('lofi')];
    case 'sleep':
      return [...getTracksByGenre('meditation'), ...getTracksByGenre('nature')];
    case 'energize':
      return [...getTracksByGenre('electronic'), ...getTracksByGenre('cinematic')];
    case 'meditate':
      return [...getTracksByGenre('meditation'), ...getTracksByGenre('ambient')];
    default:
      return MUSIC_LIBRARY;
  }
}
...
export function getAllGenres(): Track['genre'][] {
  return ['lofi', 'ambient', 'instrumental', 'classical', 'electronic', 'nature', 'meditation', 'jazz', 'cinematic'];
}
...
export function getAllMoods(): string[] {
  const moods = new Set(MUSIC_LIBRARY.map((track) => track.mood).filter(Boolean));
...
export function getLibraryStats() {
  ...
  const stats = {
    ...
    totalDuration: MUSIC_LIBRARY.reduce((sum, track) => sum + track.duration, 0),
