...
async function exportAppSettings(): Promise<Record<string, any>> {
  try {
    ...
    Logger.info('[Backup] Exported app settings:', Object.keys(settings));
    return settings;
  } catch (error) {
    ...
    if (backupData.categories) {
      for (const category of backupData.categories) {
        try {
          ...
          Logger.error(`[Backup] Failed to import category ${category.id}:`, error);
        }
      }
    }
    ...
    if (backupData.accounts) {
      for (const account of backupData.accounts) {
        try {
          ...
          Logger.error(`[Backup] Failed to import account ${account.id}:`, error);
        }
      }
    }
    ...
    if (backupData.posts) {
      for (const post of backupData.posts) {
        try {
          ...
          Logger.error(`[Backup] Failed to import post ${post.id}:`, error);
        }
      }
    }

    Logger.info('[Backup] Import completed:', imported);
