...
export const useVaultStore = create<VaultState>((set, get) => ({
  ...
  createVault: async (password: string) => {
    try {
      ...
      const { encrypted: encryptedDek, nonce: dekNonce } = await encryptDek(
        dek,
        kek,
      );
  ...
  isBiometricEnabled: async () => {
    try {
      const biometricData = await SecureStore.getItemAsync(
    ...
    try {
      ...
      try {
        if ((SecureStore as any).AFTER_FIRST_UNLOCK_THIS_DEVICE_ONLY) {
          secureStoreOptions.keychainAccessible = (
            SecureStore as any
      ...
      try {
        if ((SecureStore as any).AuthenticationType?.BIOMETRICS) {
          secureStoreOptions.authenticationType = (
            SecureStore as any
  ...
  unlockWithBiometric: async () => {
    try {
      ...
      const biometricDataStr = await SecureStore.getItemAsync(
        "biometric_vault_data",
      );
