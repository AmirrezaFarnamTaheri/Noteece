...
pub fn process_ocr_job_cmd(
    ...
    let vault_path = AppConfig::vault_path()
        .ok_or_else(|| "Vault path not configured".to_string())?;
    
    let dek = {
        let dek_guard = db.dek.lock().map_err(|_| "Failed to lock DEK".to_string())?;
        dek_guard.clone().ok_or_else(|| "DEK not available (Vault locked)".to_string())?
    };
    ...
    let temp_file_path = temp_dir.join(format!("noteece_ocr_{}.png", &blob_id[..8.min(blob_id.len())]));
...
#[tauri::command]
pub fn get_pending_ocr_jobs_cmd(db: State<DbConnection>) -> Result<Vec<core_rs::ocr::OcrResult>, String> {
    crate::with_db!(db, conn, {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            Ok(core_rs::ocr::OcrResult {
                ...
                status: status_str.parse().unwrap_or(core_rs::ocr::OcrStatus::Unknown),
...
pub async fn process_ocr_queue_cmd(
    ...
    let dek = {
        let dek_guard = db.dek.lock().map_err(|_| "Failed to lock DEK".to_string())?;
        dek_guard.clone().ok_or_else(|| "DEK not available".to_string())?
    };
    ...
    for blob_id in pending_jobs {
        ...
        // Write to temp file
        let temp_file_path = std::env::temp_dir().join(format!("noteece_ocr_{}.png", &blob_id[..8.min(blob_id.len())]));
