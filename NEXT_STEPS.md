...
*   **Audit Logs:** Create a tamper-evident, encrypted log of all data access and modification events within the vault for user review.
...
*   **RAG (Retrieval-Augmented Generation):** Use the existing FTS5 and vector embeddings (already partially implemented) to let users "Chat with their Vault".
    *   *Example:* "What was the decision we made about the database schema last month?"
...
*   **Habit Correlation:** Correlate data from the Health Hub (e.g., sleep quality) with productivity metrics (e.g., tasks completed) to offer personalized advice.
    *   *Insight:* "You complete 30% more tasks on days you sleep >7 hours."
...
### 3.1. "Zen Mode" & Focus
*   **Distraction-Free Editor:** A writing mode that fades out all UI elements, leaving only the text and a subtle word count.
...
*This document is a living roadmap. Priorities may shift based on user feedback and technical discoveries.*
