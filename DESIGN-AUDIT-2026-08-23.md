...
For an encrypted-vault productivity tool the database recommends **Minimalism/Swiss** styling on deep-blue-slate dark surfaces (`--color-background:#0F172A`, secure-green accent `#059669`, slate muted) with Inter kept for _body_ but paired with a technical display face — plus its pre-delivery checklist items that map exactly to the findings above (no emoji icons, visible focus, cursor-pointer on clickables).
