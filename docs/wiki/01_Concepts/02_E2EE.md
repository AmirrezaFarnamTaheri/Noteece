...
| :----------------------- | :--------------------- | :------------------------------- | :----------------------- |
| **Key Derivation**       | **PBKDF2-HMAC-SHA512** | Password-based Key Derivation    | `pbkdf2` (RustCrypto)    |
...
2.  **Key Encryption Key (KEK):** Derived from the password.
    - _Algorithm:_ PBKDF2-HMAC-SHA512
    - _Params:_ 256,000 iterations (tunable; kept in sync with the SQLCipher KDF setting).
...
| Threat                          | Mitigation                                                                                                |
| :------------------------------ | :-------------------------------------------------------------------------------------------------------- |
| **Stolen Device (Powered Off)** | Full Disk Encryption (OS) + SQLCipher + PBKDF2-HMAC-SHA512 (256k iterations) makes brute-force expensive. |
