...
The search system handles the unique constraint of **SQLCipher**. Since the database file is encrypted, an external search indexer (like Elasticsearch or a standard file crawler) cannot read the content. The search must happen *inside* the encrypted boundary or within the application memory.
...
A common question is: *"How does FTS index encrypted data?"*
...
3.  **Indexing:** The FTS engine reads/writes these *decrypted memory buffers*.
